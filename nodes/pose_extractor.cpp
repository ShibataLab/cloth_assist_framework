// pose_extractor.cpp: program to extract pose of mannequin
// Author: Nishanth Koganti
// Date: 2016/6/21
// Source: iai_kinect2 package
...
public:
  Receiver(const std::string &topicColor, const std::string &topicDepth, const std::string &fileName)
    : topicColor(topicColor), topicDepth(topicDepth), updateImage(false),
      updateCloud(false), save(false), running(false), frame(0), queueSize(5),
      nh("~"), spinner(0), it(nh), fileName(fileName),
      fs(fileName, std::ofstream::out)
  {
...
private:
  ...
  void cloudViewer()
  {
    ...
    visualizer->setPosition(color.cols, 0);
...
int main(int argc, char **argv)
{
  ...
  Receiver receiver(topicColor, topicDepth, fileName);

  std::cout << "\t\tMannequin Posture Extraction\t\t" << std::endl;
  std::cout << "Click on the following mannequin body parts in order: " << std::endl;
  std::cout << "Head, Left Wrist, Left Shoulder, Right Wrist, Right Shoulder" << std::endl;
  receiver.run();
